<template>
	<view class="container" :style="{height:windowHeight+'px'}">
		<image class="page-bg" :style="{height:windowHeight+'px'}"
           mode="aspectFill"
		...
		<view class="avatar-container grid justify-center" id="avatar-container"
          ...
          @touchend="touchEnd"
			...
			<text class="cuIcon-full handle circle"
            :class="{hideHandle: !showBorder}"
            id="handle"
            :style="{top:handleCenterY-10 + 'px', left:handleCenterX-10 +'px'}"></text>
			<text class="cuIcon-order cancel circle"
            v-if="isAndroid"
            :class="{hideHandle: !showBorder}"
            id="cancel"
		...
		<view class="grid justify-around action-wrapper">
			...
			<view class="grid col-2">
				<button id="btn-save"
                class="cu-btn round action-btn bg-yellow shadow"
                @click="draw">保存</button>
			</view>
			...
			<view v-for="(item,index) in imgList" :key="index" style="display: inline-flex;">
              ...
              id="cancel"
				...
				<!-- <text v-if="currentMaskId == index" style="margin-left: 55px;" class="cuIcon-question cancel circle" @click="showTips"
				 id="cancel"></text> -->
				<image class="imgList"
               :src="'/static/image/mask/'+ index +'.png'"
               :data-mask-id="index"
